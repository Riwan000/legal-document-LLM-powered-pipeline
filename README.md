...
ഇത് ഒരു ഡെമോ-റെഡി, പ്രൈവറ്റ് “Legal Document Intelligence” സിസ്റ്റമാണ്. FastAPI (backend) + Streamlit (UI) ഉപയോഗിച്ച്:
- RAG (Retrieval-Augmented Generation) വഴി ഡോക്യുമെന്റുകളിൽ നിന്ന് ബന്ധപ്പെട്ട ഭാഗങ്ങൾ കണ്ടെത്തി ഉത്തരങ്ങൾ സൃഷ്ടിക്കുക (citation സഹിതം)
- Multilingual embeddings + FAISS vector store (ഡാറ്റ ലോക്കലായി persist ചെയ്യുന്നു)
- Clause extraction + clause store
- Contract comparison (template vs contract)
- Case file summarization
- Arabic/English bilingual search

### ഇപ്പോഴുള്ള ഫീച്ചറുകൾ

- **Document upload + ingestion**: PDF/DOCX/DOC, chunking (overlap സഹിതം), content-hash document ID
- **OCR support (scanned PDFs)**: text ഇല്ലെങ്കിൽ OCR fallback
- **Multilingual vector search**: Sentence-Transformers embeddings + FAISS persistence (`data/vector_store/`)
- **RAG search**: top chunks + score + page references; (Optional) Ollama ഉപയോഗിച്ച് LLM answer
- **Clause extraction (structured) + storage**: clauses validate ചെയ്ത് `data/clause_store/` ലേക്ക് save ചെയ്യുന്നു
- **Contract comparison**: matched/modified/missing/extra clauses കാണിക്കുന്നു
- **Case summarization**: executive summary, timeline, arguments, open issues, citations
- **Bilingual search**: query language detect ചെയ്ത് Arabic/English response
...
കൂടുതൽ വിശദാംശങ്ങൾ: `START_COMPLETE_SYSTEM.md`, `TESTING_GUIDE.md`.
